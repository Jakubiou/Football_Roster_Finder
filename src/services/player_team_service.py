...
def remove_player_from_team(db, player_id):
    sql = "DELETE FROM PlayerTeam WHERE player_id = ?"
...
def change_position(db, player_id, new_position_id):
    sql = """
    UPDATE PlayerTeam
    SET position_id = ?
    WHERE player_id = ?
    """
    db.execute(sql, new_position_id, player_id)
    db.commit()
